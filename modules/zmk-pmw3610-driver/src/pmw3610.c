...
static void suppress_automouse_layer(void) {
...
static enum pixart_input_mode get_input_mode_for_current_layer(const struct device *dev) {
    ...
    // Prefer held ball-action layers (NUM/ARROW) over a lingering automouse layer.
    int best_ball_action_idx = -1;
    int best_ball_action_layer = -1;
    for (size_t i = 0; i < config->ball_actions_len; i++) {
        for (size_t j = 0; j < config->ball_actions[i]->layers_len; j++) {
            uint8_t layer = config->ball_actions[i]->layers[j];
            if (zmk_keymap_layer_active(layer) && (int)layer > best_ball_action_layer) {
                best_ball_action_layer = layer;
                best_ball_action_idx = i;
            }
        }
    }
    if (best_ball_action_idx != -1) {
        ball_action_idx = best_ball_action_idx;
#if AUTOMOUSE_LAYER > 0
        suppress_automouse_layer();
#endif
        return BALL_ACTION;
    }
